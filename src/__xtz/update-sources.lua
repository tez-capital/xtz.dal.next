...
for platform, config in pairs(platforms) do
	...
	if config.is_mac then
		if macos_octez_release then
			...
			local asset_data = extract_asset(macos_octez_release, "^octez%-dal%-node$", octez_version)
			if asset_data then
				new_platform_sources["dal-node"] = asset_data
			else
				print("  Warning: Asset octez-dal-node not found in release")
				if current_sources[platform] and current_sources[platform]["dal-node"] then
					new_platform_sources["dal-node"] = current_sources[platform]["dal-node"]
				end
			end
		else
			-- Copy existing
